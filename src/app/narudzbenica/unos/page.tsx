...
export default function NarudzbenicaUnos() {
    ...
    const [selectedArtikalID, setSelectedArtikalID] = useState<number>(0);
    const [selectedArtikal, setSelectedArtikal] = useState<ArtikalDto>();

    const [selectedDobavljacID, setSelectedDobavljacID] = useState<number>(0);
    const [selectedKatalogID, setSelectedKatalogID] = useState<number>(0);
    const [selectedZaposleniID, setSelectedZaposleniID] = useState<number>(0);
    const [kolicina, setKolicina] = useState<string>("");
    const [napomena, setNapomena] = useState<string>("");
    const [narudzbenicaID, setNarudzbenicaID] = useState<string>("");
    const [sektor, setSektor] = useState<string>("");
    const [datum, setDatum] = useState<string>(formatDate(new Date()));
    ...
    useEffect(() => {
        getAllData("http://localhost:4000/dobavljac", "array").then(
            (result: DobavljacDto[]) => {
                ...
                setSelectedDobavljacID(result[0].sifraDobavljaca);
            }
        );
        ...
        getAllData("http://localhost:4000/katalog", "array").then((result) => {
            ...
            setSelectedKatalogID(formatedResult[0].idKataloga);
        });

        getAllData("http://localhost:4000/zaposlen", "array").then(
            (result: ZaposlenDto[]) => {
                setZaposleni(result);
                setSelectedZaposleniID(result[0].idZaposlenog);
            }
        );
    }, []);

    useEffect(() => {
        if (selectedKatalogID === 0) return;

        getData(
            `http://localhost:4000/artikal/${selectedKatalogID}`,
            "array"
        ).then((result) => {
            setArtikli(result);
